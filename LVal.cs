...
public class LVal {
    ...
    /* Reading */
    private static LVal read_sym(string s, ref int i) {
        var start = i;
        
        /* While valid identifier characters */
        while (i < s.Length && "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_+-*\\/=<>!&".Contains(s[i])) {
            i++;
        }
        ...
        bool is_num = "-0123456789_".Contains(part[0]);
        for (int j = 1; j < len; j++) {
            if (!"0123456789_".Contains(part[j])) { is_num = false; break; }
        }
        if (len == 1 && part.StartsWith('-')) { is_num = false; }

        /* Add Symbol or Number as LVal */
        LVal? x = null;
        if (is_num) {
            Num v = Num.Parse(part)!;
            x = LVal.Number(v);
        } else {
            x = LVal.Sym(part);
        }
    ...
    private static LVal read_str(string s, ref int i) {
        ...
        while (s[i] != '"') {
            if (i >= s.Length) {
                return LVal.Err("Unexpected end of input");
            }
            ...
            if (c == '\\') {
                ...
                if (LVal.str_unescapable.Contains(s[i])) {        
                    c = LVal.str_unescape(s[i]);
                } else {
                    return LVal.Err($"Invalid escape sequence \\{s[i]}");
                }
            }
        ...
        var x = LVal.Str(part.ToString());
        // x.Print();
        return x;
    }

    public static LVal read_expr(string s, ref int i, char end) {
        /* Either create new qexpr or sexpr */
        LVal x = (end == '}') ? LVal.Qexpr() : LVal.Sexpr();
        
        /* While not at end character keep reading LVals */
        while (i < s.Length && s[i] != end) {
            LVal y = LVal.Read(s, ref i);
            /* If an error then return this and stop */
            if (y?.ValType == LE.LVAL_ERR) {
                return y;
            } else {
                x.Add(y!);
            }
    ...
    public static LVal Read(string s, ref int i) {
        ...
        else if ("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_+-*\\/=<>!&".Contains(s[i])) {
            x = read_sym(s, ref i);
        }
        ...
        /* Encountered some unexpected character */
            ...
            x = Err($"Unexpected character {s[i]}");
        }
